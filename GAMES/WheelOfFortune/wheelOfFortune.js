...
for (let i = 0; i < words.length; i++) {
	board.push(' '.repeat(words[i].length).split(''));
}
log(board);
...
async function addLetter() {
	for (let a = 0; a < phrase.length - words.length + 1; a++) {
		let letter = ' ';
		let i;
		let j;
		while (letter == ' ') {
...
async function buzz() {
	...
	if (answer == phrase) {
		await alert('You won!', 18, 2);
		score = score + lettersleft;
		newgame();
	} else {
		score = score - 1;
